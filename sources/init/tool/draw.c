...
void draw(CHAR_INFO consoleBuffer[HIGHT][WIDTH], info tank[]){
    ...
    for (i = 0; i < 6; i++){
        if (tank[i].direc == TOP){
            for (j = 0; j < 12; j++){
                consoleBuffer[tank[i].line + tank_top[j].x][tank[i].column + tank_top[j].y].Attributes = BACKGROUND_BLUE;
            }
        }
        else if (tank[i].direc == RIGHT){
            for (j = 0; j < 12; j++){
                consoleBuffer[tank[i].line + tank_right[j].x][tank[i].column + tank_right[j].y].Attributes = BACKGROUND_BLUE;
            }
        }
        else if (tank[i].direc == DOWN){
            for (j = 0; j < 12; j++){
                consoleBuffer[tank[i].line + tank_down[j].x][tank[i].column + tank_down[j].y].Attributes = BACKGROUND_BLUE;
            }
        }
        else if (tank[i].direc == LEFT){
            for (j = 0; j < 12; j++){
                consoleBuffer[tank[i].line + tank_left[j].x][tank[i].column + tank_left[j].y].Attributes = BACKGROUND_BLUE;
            }
        }
    }
}
